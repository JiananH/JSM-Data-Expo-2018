...
forecast_dat <- read.table("forecast.dat", sep = " ", stringsAsFactors=FALSE)

## to do list 1
locations$cityID <- 1:nrow(locations)

names(forecast_dat) <- c("cityID", "forecastFOR", "value", "variable", "forecastON")

#"2016-09-02" "2017-09-06"
forecast=forecast_dat[as.Date(forecast_dat$forecastFOR)>="2016-09-02"&as.Date(forecast_dat$forecastFOR)<="2017-09-06",]
...
bubble1
ggsave("bubblePlot_org.pdf",bubble1)
...
ggsave("Max_Temp_org.pdf",maxTempBubble, width = 7, height = 7/2)
...
ggsave("Min_Temp_org.pdf",minTempBubble, width = 7, height = 7/2)
